...
for f in sorted(os.listdir(IMG_DIR)):
    ...
    blue_mask = cv2.inRange(hsv, np.array([90, s_min, 50]), np.array([150, 255, 255]))
    k = np.ones((2,2), np.uint8)
    blue_mask = cv2.dilate(blue_mask, k, 1)
    blue_mask = cv2.erode(blue_mask, k, 1)
    # Find the largest non-boundary contour to get center estimate
    ...
    # For each expected axis angle, find the farthest blue pixel along that ray
    # Axes are at angles: slice * i * 2, where slice = pi / N
    slice_angle = math.pi / N
    axis_dists = []
    for i in range(N):
        angle = slice_angle * i * 2
        # Scan outward from center
        max_d = min(w, h)  # max scan distance
        found_d = 0
        for d in range(5, int(max_d), 1):
            ...
            if blue_mask[py, px] > 0:
                ...
                # Once we leave the blue region, we've crossed the polygon boundary
                if found_d > 0:
                    break
    ...
    print(hdr)
    print("  " + "-" * 58)
    errs = []; w_det_sum = 0; total_cr = sum(c["credit"] for c in gt)
    ...
    for i in range(N):
        ...
        print("  {:>8.1f}  {:>6.0f}  {:>7.2f}  {:>7.2f}  {:>+7.2f}  {}".format(
            math.degrees(slice_angle * i * 2), axis_dists[i], ds, gs, err, nm))
